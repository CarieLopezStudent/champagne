(function() {
  ...
  $.extend(REPLIT, number = 0, {
    ...
    replay: function(number) {
      ...
      return this.Evaluate(filterXSS(editor.getSession().getValue()));
    },
    transfer: function(number) {
      var editor;
      editor = ace.edit('editor' + number);
      REPLIT.editor.getSession().doc.setValue(filterXSS(editor.getSession().getValue()));
    ...
    append: function(number) {
      ...
      REPLIT.editor.getSession().insert(position, filterXSS(editor.getSession().getValue()));
      return REPLIT.editor.focus();
    }
  });
